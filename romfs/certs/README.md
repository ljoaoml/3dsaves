...
HTTPS requests to Dropbox's API domains verify correctly even though the
3DS's built-in trusted cert list (`SSLC_DefaultRootCert_*` in libctru) is
old and does not cover every modern CA/chain. `source/http.c` loads all of
these at startup and adds them as trusted roots on every HTTPS request via
